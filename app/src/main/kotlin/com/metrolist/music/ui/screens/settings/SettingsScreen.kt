...
fun SettingsScreen(
    ...
    val spotifyPickerLauncher = rememberLauncherForActivityResult(
        contract = ActivityResultContracts.OpenDocument()
    ) { uri ->
        if (uri != null) {
            coroutineScope.launch {
                val result = PlaylistConverter.parseExportifyUri(context, uri)
                result.fold(
                    onSuccess = { playlists ->
                        ...
                        Toast.makeText(context, "Importing ${playlists.size} Spotify playlist(s)...", Toast.LENGTH_SHORT).show()
                    },
                    onFailure = { err ->
                        Toast.makeText(context, "Failed to import: ${err.message}", Toast.LENGTH_LONG).show()
                    }
                )
            }
        }

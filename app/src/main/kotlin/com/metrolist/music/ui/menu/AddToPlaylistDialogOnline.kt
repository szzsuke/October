...
fun AddToPlaylistDialogOnline(
    ...
    val syncUtils = com.metrolist.music.LocalSyncUtils.current
    ...
    androidx.compose.runtime.LaunchedEffect(isVisible) {
        if (isVisible && !com.metrolist.music.ui.component.LoginPromptManager.isUserLoggedInState) {
            onDismiss()
            com.metrolist.music.ui.component.LoginPromptManager.triggerLoginPrompt()
        }
    }
    if (!com.metrolist.music.ui.component.LoginPromptManager.isUserLoggedInState) return
                    ...
                    .clickable {
                        ...
                        coroutineScope.launch(Dispatchers.IO) {
                            ...
                            try {
                                ...
                                val resolved = resolvedSongIds.filterNotNull()
                                database.addSongsToPlaylist(
                                    playlist,
                                    resolved.map { it to null },
                                    prepend = addToPlaylistPosition.prepend,
                                )
                                playlist.playlist.browseId?.let { browseId ->
                                    syncUtils.scheduleAddToPlaylist(browseId, playlist.id, resolved)
                                }
                            } finally {
                                                        ...
                                                        if (items.isNotEmpty()) {
                                                            ...
                                                            if (firstSong != null) {
                                                                ...
                                                                withContext(Dispatchers.IO) {
                                                                    try {
                                                                        database.insert(firstSongMedia)
                                                                        val toggled = firstSongEnt.toggleLike()
                                                                        database.query {
                                                                            update(toggled)
                                                                        }
